...
sap.ui.define([
	...
	return BaseController.extend("ts.controller.Master", {
		...
		onInit: function() {
			...
			this._oGroupSortState = new GroupSortState(oViewModel, grouper.Price(this.getResourceBundle()));
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("Carrid", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("ts.view.ViewSettingsDialog", this);
				...
				function(mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							Carrid: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("Carrid")),
							Connid: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("Connid")),
							Fldate: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("Fldate"))
						}, true);
					}
				}.bind(this),
				function(mParams) {
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				Carrid: encodeURIComponent(oItem.getBindingContext().getProperty("Carrid")),
				Connid: encodeURIComponent(oItem.getBindingContext().getProperty("Connid")),
				Fldate: encodeURIComponent(oItem.getBindingContext().getProperty("Fldate"))
			}, bReplace);
		},
