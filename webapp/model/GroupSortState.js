sap.ui.define([
	...
	return BaseObject.extend("ts.model.GroupSortState", {
		...
		/**
		 * Sorts by Carrid, or by Paymentsum
		 *
		...
		/**
		 * Groups by Paymentsum, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Paymentsum") {
				...
				this._oViewModel.setProperty("/sortBy", "Paymentsum");

				aSorters.push(
					new Sorter("Paymentsum", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Carrid");
			}
